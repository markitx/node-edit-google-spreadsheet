...
exports.returnSheets = function (opts, callback) {
  if (!callback)
    callback = opts.callback;
  if (!callback)
    throw "Missing callback";
  ...
  auth(opts, function(err, token) {
    ...
    spreadsheet.setToken(token);
    if (!opts.spreadsheetId && !opts.spreadsheetName) {
      spreadsheet.getSheetInfos('spread', callback);
    }
    else if (!opts.worksheetId && !opts.worksheetName) {
      spreadsheet.getSheetId('spread', function(err) {
        if (err) return callback(err, null);
        spreadsheet.getSheetInfos('work', callback);
      });
    }
    else {
      spreadsheet.log('Worksheet and spreadsheet information already specified.');
      spreadsheet.getSheetInfos('work', function (err, sheetInfo) {
        if (err) return callback(err, null);
        var newProcessedInfo = {};
        for (var k in sheetInfo.processed) {
          if ((opts.worksheetName == k) || (opts.worksheetId == sheetInfo.processed[k])) {
            newProcessedInfo[k] = sheetInfo.processed[k];
          }
        }
        sheetInfo.processed = newProcessedInfo;
...
Spreadsheet.prototype.getSheetInfos = function(type, callback) {
  var _this = this;
  var id = type + 'sheetId';
  var spreadsheetUrlId = type === 'work' ? ('/' + _this.spreadsheetId) : '';

  var sheetInfos = {
    raw: {},
    processed: {} // a dictionary consisting of sheetName:sheetId pairs
  };
  _this.request({
    url: _this.protocol + '://spreadsheets.google.com/feeds/' +
    ...
    sheetInfos.raw[type + 'sheets'] = entries.map(function(e1) {
      var e2 = {};
      for (var prop in e1) {
        var val = e1[prop];
      ...
      if (e2.title && e2.id && /([^\/]+)$/.test(e2.id)) {
        sheetInfos.processed[e2.title] = RegExp.$1;
      }
      return e2;
    });
    _this.log(type + 'sheets:');
    for (var k in sheetInfos.processed) {
      _this.log(k + ': ' + sheetInfos.processed[k]);
    }
    callback(null, sheetInfos);
  });
}
...
Spreadsheet.prototype.getSheetId = function(type, callback) {
  ...
  _this.getSheetInfos(type, function (err, sheetInfos) {
    for (var k in sheetInfos.raw) {
      if (sheetInfos.raw.hasOwnProperty(k)) {
        _this.raw[k] = sheetInfos.raw[k];
      }
    }
    if (sheetInfos.processed[name]) {
      _this[id] = sheetInfos.processed[name];
      return callback(null);
    }
